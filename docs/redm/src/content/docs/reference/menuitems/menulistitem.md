...
:::caution
Color panels are not available in RedM. This section is FiveM-only. The `ShowColorPanel` and `ColorPanelColorType` properties exist in the RedM build, but setting them has no effect.
:::
...
:::caution
Opacity panels are not available in RedM either. `ShowOpacityPanel` exists in the RedM build, but setting it has no effect.
:::
