...
1. Use the `MenuAPI.RedM` NuGet package, which can be found [here](https://www.nuget.org/packages/MenuAPI.RedM/). This is the easiest way to get started.
2. Download the latest release zip and use the RedM DLL for your resource. Simply include the DLL as a reference in your C# project and add `using MenuAPI;` to each file where you need to use MenuAPI.

If you're building for FiveM instead, you'll want the [MenuAPI.FiveM](https://www.nuget.org/packages/MenuAPI.FiveM/) package.
