...
:::note
Not every item type is available in RedM, and `RightIcon` and `Label` are unavailable on the item types that use the right side of the row themselves. Each page lists its own limitations.
:::
