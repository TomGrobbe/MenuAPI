...
|HeaderTexture|KeyValuePair&lt;string,&nbsp;string&gt;|(empty)|A custom banner image. The `Key` is the streamed texture dictionary, the `Value` is the texture name. The texture dictionary is requested for you. When this is not set, the default banner is used. In NUI render mode you can also replace a banner with an image file instead of a game texture, see [Theming and banners](../theming/#part-two-banner-images).|Yes|
|HeaderImage|string?|`null`|A banner image your resource ships, named whatever you like, in its `menuapi-banners` folder. Set it to the name without the extension, so `"outlaw"` uses `menuapi-banners/outlaw.png`. Wins over `HeaderTexture`, and falls back to it when the file is not there. NUI render mode only, see [Theming and banners](../theming/#option-two-a-menu-pointing-at-your-own-image).|Yes|
